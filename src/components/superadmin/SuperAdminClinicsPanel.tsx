...
export default function SuperAdminClinicsPanel() {
  ...
  const openDetail = async (clinicId: string) => {
    ...
    try {
      ...
      toast.error(err instanceof ApiError ? err.message : "Failed to load clinic");
    } finally {
  ...
  const handleActivate = async () => {
    ...
    try {
      ...
      toast.success(res.message || "Clinic activated.");
      await refreshDetail();
    } catch (err) {
      toast.error(err instanceof ApiError ? err.message : "Failed to activate clinic");
    } finally {
      setActionBusy(false);
    }
  };
  ...
  const handleDeactivate = async () => {
    ...
    try {
      ...
      toast.success(res.message || "Clinic deactivated.");
      setDeactivateOpen(false);
      setDeactivateReason("");
      await refreshDetail();
    } catch (err) {
      toast.error(err instanceof ApiError ? err.message : "Failed to deactivate clinic");
    } finally {
      setActionBusy(false);
    }
  };
  ...
  const handleExtend = async () => {
    ...
    try {
      ...
      toast.success(res.message || "Subscription extended.");
      setExtendOpen(false);
      setExtendMonths(1);
          ...
          <div className="max-h-[75vh] space-y-5 overflow-y-auto">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                ...
                {[detail.location?.city, detail.location?.district].filter(Boolean).join(", ") || "—"}
              </p>
            </div>

            {/* Licenses */}
            <div className="rounded-xl border border-gray-200 p-4 text-sm dark:border-gray-800">
              <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-400">Licenses</p>
              <ul className="space-y-1 text-gray-600 dark:text-gray-400">
                <li>Trade license: {detail.licenses?.trade_license_number || "—"}</li>
                <li>Drug license: {detail.licenses?.drug_license_number || "—"}</li>
                <li>Clinical establishment: {detail.licenses?.clinical_establishment_reg_number || "—"}</li>
              </ul>
            </div>
                ...
                <dl className="grid gap-x-6 gap-y-2 text-sm sm:grid-cols-2">
                  <div>
                    ...
                    <dd className="text-gray-800 dark:text-white/90">
                      {formatCurrency(sub.monthly_amount, sub.currency)} / month
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs text-gray-400">Period</dt>
                    <dd className="text-gray-800 dark:text-white/90">
                  ...
                  <div>
                    <dt className="text-xs text-gray-400">Blocked</dt>
                    <dd className="text-gray-800 dark:text-white/90">
                      {sub.blocked ? `Yes — ${sub.blocked_reason ?? ""}` : "No"}
                    </dd>
                  </div>
                </dl>
            ...
            <div>
              ...
              {payments.length === 0 ? (
                <p className="py-3 text-sm text-gray-500 dark:text-gray-400">No payments.</p>
              ) : (
                <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-800">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableCell isHeader className="px-3 py-2 text-left text-xs font-semibold uppercase text-gray-500">Invoice</TableCell>
                        <TableCell isHeader className="px-3 py-2 text-left text-xs font-semibold uppercase text-gray-500">Amount</TableCell>
                        <TableCell isHeader className="px-3 py-2 text-left text-xs font-semibold uppercase text-gray-500">Status</TableCell>
                        <TableCell isHeader className="px-3 py-2 text-left text-xs font-semibold uppercase text-gray-500">Created</TableCell>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {payments.map((p) => (
                        <TableRow key={p.id}>
                          ...
                          <TableCell className="px-3 py-2 text-sm text-gray-800 dark:text-white/90">
                            {formatCurrency(p.amount, p.currency)} · {p.months}mo
                          </TableCell>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <QuickFact label="Branches" value={detail.branches?.length ?? 0} />
              <QuickFact label="Staff" value={detail.staff?.length ?? 0} />
              <QuickFact label="Doctors" value={detail.doctors?.length ?? 0} />
            </div>
          </div>
        )}
      </Modal>
