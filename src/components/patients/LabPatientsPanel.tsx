...
// Patients seen through lab test bookings. The backend dedupes by the actual
// patient's identity (GET /branches/{id}/lab-patients) — this panel used to list
// raw lab appointments, which double-counted anyone with more than one booking and
// attributed reception-made bookings to the staff account that created them.
export default function LabPatientsPanel() {
  ...
  const load = useCallback(
    async (b: BranchSelectValue | null, nextOffset: number, append: boolean) => {
      ...
      setError(null);
      try {
        const res = await patientsApi.listLabByBranch(b.id, {
