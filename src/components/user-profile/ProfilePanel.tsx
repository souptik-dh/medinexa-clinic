      ...
      {user?.role === "branch_staff" && (
        <div className="col-span-12 rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
          <div className="mb-4 flex items-center justify-between">
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">{t("profilePage.permissionsGranted")}</h3>
          </div>
          <div className="mt-2 flex flex-wrap gap-2">
            {(user?.permissions ?? []).length === 0 ? (
              <span className="text-sm text-gray-500 dark:text-gray-400">{t("profilePage.noPermissionsGranted")}</span>
            ) : (
      ...
      {isBranchStaff ? (
        ...
        <div className="col-span-12 rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6 xl:col-span-8">
          <h3 className="mb-5 text-lg font-semibold text-gray-800 dark:text-white/90 lg:mb-7">
            {t("profilePage.myClinicAndBranch")}
          </h3>
          {!staffClinic || !staffBranch ? (
            <div className="rounded-xl bg-gray-50 p-6 text-center text-sm text-gray-500 dark:bg-gray-800/50 dark:text-gray-400">
              {t("profilePage.couldNotLoadBranch")}
            </div>
          ) : (
            <div className="rounded-2xl border border-gray-200 p-5 dark:border-gray-800">
              ...
              <div className="mt-4 border-t border-gray-100 pt-4 dark:border-gray-800">
                <p className="text-theme-xs text-gray-400 dark:text-gray-500">{t("appointments.branch")}</p>
                <h4 className="font-semibold text-gray-800 dark:text-white/90">
                  {staffBranch.name}
                </h4>
        ...
        <div className="col-span-12 rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6 xl:col-span-8">
          ...
          {loading ? (
            ...
            <div className="rounded-xl bg-gray-50 p-6 text-center text-sm text-gray-500 dark:bg-gray-800/50 dark:text-gray-400">
              {clinic
                ? t("profilePage.clinicNotSetUp")
                : t("profilePage.noClinicsFound")}
            </div>
          ) : (
              ...
              {clinics.map((c) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <Badge color="info">{t("branchesListPage.branchesCountBadge", { count: c.branch_count ?? 0 })}</Badge>
                  </div>
